...
public enum VanillaPaintings implements Font, ComponentLike {
    ...
    VanillaPaintings(final Supplier<Art> artSupplier, final int character, final int pixelWidth) {
    ...
    public static VanillaPaintings of(Art art) {
        for (var it : values()) {
            if (it.getArt() == art) return it;
        }
        return null;
    }

    public static Component componentOf(Art art) {
        VanillaPaintings inst = of(art);
        return inst != null
            ? inst.component
            : Component.empty();
    }
